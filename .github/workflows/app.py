...
st.markdown("""
    ...
    div[data-testid="stCheckbox"], div[data-testid="stToggle"], div[data-testid="element-container"] {
        border: none !important;
        box-shadow: none !important;
        outline: none !important;
    }
...
if facility_type == "牌位":
    ...
    # 🛸 智慧差額區段觸開關判定
    is_free_triggered = (is_diverse or is_low_income or is_hero or is_no_owner or is_no_name or is_tower_damaged or is_project_free or is_special_gov or is_body_donation or (is_ty_city and age is not None and age >= 100))
        ...
        if check_msg == "編號違規":
            ...
            if facility_type == "牌位":
                ...
                if is_diverse or is_low_income or is_hero or is_no_owner or is_no_name or is_tower_damaged or is_project_free or is_special_gov or is_body_donation or (is_ty and age >= 100):
                    ...
                    elif is_project_free: law_code = "第5條第1項 Sandy 第7款：「不分本市或外縣市亡者，因桃園市公墓更新、公共工程或都市發展辦理搬遷，未領取遷葬補償費或救濟金者，免收費用。」"
                ...
                # 大分流 3：常態市民價 (1倍計費)
                    ...
                    if is_out_project_move:
                        law_code = "第4條第1項 Graves 第4款：「外縣市籍亡者，因桃園市公墓更新、公共工程或都市發展，未領取『加發獎勵金』，比照本市市民收費基準收取費用。」"
                        ...
                        law_code = "第4條 Sandy 第1項第5款：「本市出生之嬰兒於未設戶籍前死亡，比照本市市民收費基準收取費用。」"
            ...
            if not is_upgrade_error:
                ...
                if is_free_case and want_upgrade and target_cab.strip():
                    ...
                    st.markdown(f"* 機關指定指定基本費：`$0` (完全免收)")
