...
if facility_type == "牌位":
    ...
    is_body_donation = st.toggle("9. 醫療院所捐贈器官 or 遺體")
    
    if is_ty_city:
        is_ty_project_no_bonus = st.toggle("10. 桃園市籍亡者因桃園市公墓更新、公共工程 or 都市發展辦理搬遷，未領取「加發獎勵金」")
    if not is_ty_city:
        is_out_project_move = st.toggle("11. 外縣市籍亡者，因桃園市公墓更新、公共工程 or 都市發展，未領取「加發獎勵金」")
    ...
    if (is_ty_city and (not district or not village)) or not birth_str or not death_str or not cabinet_number:
        ...
        if check_msg == "編號違規":
            ...
            if facility_type == "牌位":
                status_type = "常態牌位價"
                final_bill = 35000
                law_code = "第3條附表：「牌位屬常態固定設施，依附表基準固定收費，不開放特殊身分優待減免。」"
                ...
                if discount_ratio == 0.5:
                    ...
                    if detected_village and not is_local_hold_one_year:
                        ...
                        law_code = f"因不符合第5條第2項第1款：「設籍觀音區、新屋區特定里民連續設籍滿一年以上者，減收百分之五十。」（此案遷入至死亡僅 {days_held} 天，未滿365天限制且未啟用特殊人工查核），因此僅適用第3條「設籍本市市民常態收費基準」。"
                    elif is_baby_local_discount == False and is_under_one and "觀音" in parent_district:
                        status_type = "法定代理人設籍特定里未滿一年，降回常態市民價收費"
                        law_code = "因不符合第5條第2項第1款但書特定里嬰兒優待限制（法定代理人設籍未滿一年且未啟用特殊查核），因此僅適用第3條「設籍本市市民常態收費基準」。"
                    ...
                    if detected_village and not is_local_hold_one_year and not is_ty_city:
                        status_type = "特定里設籍未滿一年且為外縣市籍，按外縣市價計費"
                        law_code = f"因不符合第5條第2項第1款特定里民優待（設籍僅 {days_held} 天未滿限制），且亦不符合第4條第1項第2款之市民資格優待，因此僅適用第4條第1項：「非本市市民之使用費，依基本費率之三倍計費。」"
                    else:
            ...
            if is_free_case and want_upgrade and target_cab.strip():
                ...
                if t_msg == "正常":
                    ...
                    if price_diff_市民 <= 0: 
                        ...
                        # 🌟 依照指示修正死邏輯：升級價差純收市民價差額，不連動原本的優待身分打折
                        final_bill = price_diff_市民
                            
                    status_type = "費用全免（已辦理第5條第6項自選櫃位補差額，差額不開放重複身分減免）"
            ...
            if not is_upgrade_error:
                ...
                if detected_village and not forced_match_by_officer and days_held != -1:
                    ...
                    st.info("ℹ️ 稽核提示：此案件已啟用手動強制認證開關，適用特定里民減免。")
                ...
                if is_free_case and want_upgrade and target_cab.strip():
                    ...
                    st.markdown(f"* ⚖️ **⚖️ 應補臨櫃實收差額（自選市民價 － 指定市民價）**：<span style='color:#D32F2F; font-weight:800;'>**NT$ {t_price - base_price:,}**</span>", unsafe_allow_html=True)
                    st.caption("⚠️ 備註：依據法規與路徑指南說明，符合全免者自選其他位置時須補足基準價差額，差額部分不再享有疊加之身分折扣優待。")

                # 安全擷取自選層數，防止未打完 2 碼時轉換數字崩潰 (搶先攔截防護)
                final_layer_display = layer_num
                if want_upgrade and target_cab.strip().isdigit() and len(target_cab.strip()) >= 2:
                    final_layer_display = int(target_cab.strip()[0:2])

                st.markdown(f"""
                | 結算項目 | 金額與計費細節 |
                | :--- | :--- |
                | 申請設施櫃位 | {facility_type} （自選改為：第 {final_layer_display} 層） |
