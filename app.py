...
# ==========================================
# 3. 勾選符合之特殊減免條件（🌟 核心優化：變數生命週期完全拉出，防崩潰消失）
# ==========================================
...
# 預先在最外層將所有 Toggle 開關變數初始化宣告，絕對不包在 if 裡面
...
if facility_type == "牌位":
    ...
    # 1~9 條常態開關，永久穩定顯示
    is_diverse = st.toggle("1. 非桃園市亡者使用多元葬法專專區")
    ...
    is_baby_local_discount = True if (is_under_one and is_ty_city and parent_detected_village) else False
        ...
        if check_msg == "編號違規":
            ...
            if not is_upgrade_error:
                ...
                if is_free_case and want_upgrade and target_cab.strip():
                    ...
                    st.markdown(f"* 家屬自選新櫃位（第 {int(target_cab[0:2])} 層）標準市民價：`NT$ {t_price:,}`")
